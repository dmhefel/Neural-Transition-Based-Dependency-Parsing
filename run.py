...
def train_for_epoch(parser, train_data, dev_data, batch_size):
    ...
    for i, (train_x, train_y) in enumerate(minibatches(train_data, batch_size)):
        ...
        outputs = parser.model.forward(train_x) #list of outputs from forward algorithm
        yhat = outputs[-1] #get rid of bias row
        loss+= -np.sum(train_y*np.log(yhat)/batch_size) #This might be wrong
        lr = parser.model.lr #learning rate of model

        delta_o = yhat-train_y
        delta_h2 = np.multiply(np.dot(delta_o, np.transpose(parser.model.U[:-1])), d_relu(outputs[-2][:,:-1]))
        delta_h1 = np.multiply(np.dot(delta_h2, np.transpose(parser.model.W2[:-1])), d_relu(outputs[-3][:,:-1]))
        h2_gradient = np.dot(np.transpose(outputs[1]), delta_h2)
        h1_gradient = np.dot(np.transpose(outputs[0]), delta_h1)
        U_gradient = np.dot(np.transpose(outputs[2]), delta_o)
        parser.model.W2 -= (lr*h2_gradient)
        parser.model.W1 -=  (lr*h1_gradient)
        parser.model.U -= (lr* U_gradient)
