...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.dependencies = [] #empty list for dependencies
        self.stack = ['ROOT'] #start with only root in stack
        self.buffer = self.sentence[:] #copy sentence
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses)>0:
        predictions = model.predict(unfinished_parses[:batch_size])
        for prediction, parse in zip(predictions, unfinished_parses[:batch_size]):
